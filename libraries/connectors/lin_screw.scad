...
module segment_with_square_nuts(length, height, width, nut_height, nut_width, screw_d, clearance, anchor = CENTER, spin = 0, orient = UP) {
    assert(nut_height < height, "Nut height must be less than segment height");
    eps = 0.01;
    attachable(anchor = anchor, spin = spin, orient = orient, size = [length * 7/9 , width, height]) {
        union() {
            right(-_length/2) back(-_width/2) up(-_height/2) difference() {
                union() {
                    ...
                    up(height/2)linear_extrude(height/2)
                    polygon(points = [
                        [0,0],
                        [length, 0],
                        [length, width * 1/2],
...
module segment_compliment_with_bolts(length, height, width, screw_head_h, screw_head_d, screw_d, clearance, anchor = CENTER, spin = 0, orient = UP) {
    ...
    attachable(anchor = anchor, spin = spin, orient = orient, size = [length, width, height]) {
        union() {
            zrot(180) right(-length/2) back(-width/2) up(-height/2) difference() {
                ...
                union(){
                    right(length * 6.5/9) back(screw_head_h / 2) up(height/2) xrot(90) {
                        up(eps)cylinder(h = screw_head_h, d = screw_head_d, center = true, $fn = 50);
                        up(-width/2) cylinder(h = width, d = screw_d, center = true, $fn = 50);
                    }
                    right(length * 2.5/9) back(screw_head_h / 2) up(height/2) xrot(90) {
                        up(10 * eps)cylinder(h = screw_head_h, d = screw_head_d, center = true, $fn = 50);
                        up(-width/2) cylinder(h = width, d = screw_d, center = true, $fn = 50);
                    }
                }
            }
